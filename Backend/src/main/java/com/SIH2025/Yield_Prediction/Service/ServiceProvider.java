...
public class ServiceProvider {
    ...
    public float giveYieldPrediction(String state, float acres, String crop) {
        return 0;
    }

    public Map<String,Integer> giveCropRecommendation(String state, float acres) {
        return null;
    }
    ...
    public float getPrediction(String state, float area, String crop) {
        ...
        Map response =  restTemplate.postForObject(FASTAPI_URL, payload, Map.class);
        if (response != null && response.containsKey("prediction")) {
            return ((Number) response.get("prediction")).floatValue();
        }
        return 0;
    }
